...
class UTEScraper:
    ...
    async def _login(self, page: Page) -> bool:
        ...
        try:
            ...
            # Do not wait for UTE's occasionally stalled navigation. The
            # authenticated-session indicator below is the actual success test.
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:
                        ...
                        # Reset the full context: a failed provider redirect
                        # can leave cookies and in-flight navigation unusable.
                        await context.close()
                        context = await self._new_context(browser)
                        page = await context.new_page()
